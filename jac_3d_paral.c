...
#define EXTRALARGE_TSTEPS  10   // (мы уменьшали, чтобы быстрее уложиться в лимиты)
...
static double kernel_jacobi_3d_for(int n, int itmax)
{
    ...
    #pragma omp parallel default(none) shared(A,B,n,itmax,inv_12,eps)
    {
        for (int it = 1; it <= itmax; ++it) {
            ...
            // обнуляем eps один раз на итерацию
            #pragma omp single
            eps = 0.0f;
